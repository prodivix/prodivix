...
if (!databaseUrl) {
  console.error(
    'PRODIVIX_BACKEND_POSTGRES_TEST_URL is required for the G4 V7 PostgreSQL product Gate.'
  );
  process.exit(1);
}

const repoRoot = fileURLToPath(new URL('..', import.meta.url));
const requiredTest = 'TestAgentProductPostgreSQLGate';
const child = spawn(
  'go',
  [
    'test',
...
child.once('close', (code, signal) => {
  ...
  if (!observedRun || !observedPass) {
    console.error(
      `The G4 V7 PostgreSQL Gate did not execute and pass ${requiredTest}.`
    );
    process.exitCode = 1;
  }
});
