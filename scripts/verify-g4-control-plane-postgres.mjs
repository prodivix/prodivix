...
if (!databaseUrl) {
  console.error(
    'PRODIVIX_BACKEND_POSTGRES_TEST_URL is required for the G4 V4 PostgreSQL control-plane Gate.'
  );
  process.exit(1);
}

const repoRoot = fileURLToPath(new URL('..', import.meta.url));
const requiredTest = 'TestAgentControlPlanePostgreSQLGate';
const child = spawn(
  'go',
  [
    'test',
...
child.once('error', (error) => {
  console.error(
    `Unable to start the G4 V4 PostgreSQL control-plane Gate: ${error.message}`
  );
  process.exitCode = 1;
});
child.once('close', (code, signal) => {
  ...
  if (!observedRun || !observedPass) {
    console.error(
      `The G4 V4 PostgreSQL Gate did not execute and pass ${requiredTest}.`
    );
    process.exitCode = 1;
  }
});
