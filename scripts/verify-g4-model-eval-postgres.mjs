...
const requiredTest = 'TestAgentModelEvaluationPostgreSQLGate';
const child = spawn(
  'go',
  ['test', './internal/modules/agent', '-run', `^${requiredTest}$`, '-count=1', '-v'],
  {
...
const forward = (stream, destination) => {
  ...
  stream.on('data', (chunk) => {
    ...
    observedRun ||= new RegExp(`^=== RUN\\s+${requiredTest}$`, 'mu').test(outputTail);
    observedPass ||= new RegExp(`^--- PASS: ${requiredTest}\\b`, 'mu').test(outputTail);
  });
};
...
child.once('close', (code, signal) => {
  ...
  if (!observedRun || !observedPass) {
    console.error(`The G4 V8 PostgreSQL Gate did not execute and pass ${requiredTest}.`);
    process.exitCode = 1;
  }
});
