...
- 日期：2026-07-31
- ImplementationStatus：V7 Implemented
- ProductGateStatus：Local Passed / CI Configured / durable Evidence pending
...
- [x] Scenarios/Verification/Execution Center/Issues使用同一revision/plan/run/check/evidence identity。
- [x] matrix、Impact selection reason、stale/unstable/missing和SourceTrace可导航。
- [x] BHV/VER诊断前后端共享，TST保持原语义。
- [x] CLI/CI strict contract、exit code、idempotent upload和credential hard cut完成。
- [x] refresh/crash/ACK loss恢复不重复Evidence或把中断run标passed。
- [x] 产品不自动写Workspace，也不提前实现G4/G5/G6能力。
