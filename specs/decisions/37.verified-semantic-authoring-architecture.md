...
- 日期：2026-07-14
- ImplementationStatus：G1 Semantic Index Foundation Implemented / Broader Architecture In Progress
...
4. [已完成] Web 通过 Browser Adapter 组合 NodeGraph 执行能力。

### A2：Animation

1. [已完成] 创建可独立 build/test 的 `@prodivix/animation`，由其拥有 contract、确定性 normalizer、作者态工厂、keyframe 与 timeline evaluator。
2. [已完成] Web 与 Compiler 共享 `@prodivix/animation` 的 keyframe 与 timeline evaluator。
...
1. [待开始] 建立 revision-bound Workspace Semantic Index，并接入首批领域 provider。
2. [待开始] 以 Language Service 作为 Code Semantic Provider，贯通跨领域 definition、reference、completion、diagnostics 与 impact query。
3. [待开始] 落地 Component Definition、Public Contract、Component Instance、依赖 DAG 与原子 subtree extraction。
4. [待开始] 建立一等 Collection、稳定 item/index scope 和显式状态 regions。
5. [待开始] 建立 Component/Collection Preview、Export、SourceTrace、Issues 与 Golden conformance。
...
4. [基线已完成] `@prodivix/golden-conformance` 已将 Golden App 建成 Living Conformance 基线，覆盖 create、edit、History、Atomic Commit save planning、Outbox/local replica recovery、显式 conflict resolution、同一 `pir-component` 作为 route-level artifact 的双 route 单模块复用、full-workspace React/Vite export 与进程内 Vite build。
5. [待开始] 将 Golden 扩展到 Blueprint Component Instance、subtree extraction、独立项目 install、typecheck、test、runtime behavior、browser smoke 与 visual regression，并把运行结果固化为正式 `VerificationEvidence`。
...
2. 建立 Verification Evidence、ExecutionProvider、Workspace Semantic Index 和后续 Intelligence projections 需要新的基础设施投入。
