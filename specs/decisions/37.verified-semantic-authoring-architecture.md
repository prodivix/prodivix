...
- Accepted
- 日期：2026-07-13
- 实现状态：G0 Kernel Implemented / Broader Architecture Partial
...
PIR 可以引用代码，但不吞并真实源码。Code-owned 内容通过 Code Authoring Environment 提供编辑、索引、诊断、预览、定位和 AI patch。
...
1. UI Graph、Route、Component、CodeArtifact、NodeGraph 和 Animation 的作者态语义。
2. Stable identity、Reference、Scope、Slot、Target 和 SourceSpan。
3. Domain Codec、Normalizer、Validator、Command planner 和 projection。
4. Component Contract、Data Contract、Behavior Scenario 等跨视图语义引用。
...
四个视图不直接互相扫描内部状态。它们通过 Workspace、Authoring Environment、Runtime contracts 和 Diagnostics 进行协作。
...
- `@prodivix/authoring` 的稳定查询层。
- `@prodivix/ai` 的模型无关计划、上下文、tool 和 trace 层。
- 后续 Workspace Intelligence Index。

### 拥有

1. Symbol、Reference、Dependency、Route、Component、Behavior、Token 和 Test link index。
2. Context Pack Builder、impact analysis、change planning 和 migration assistance。
...
1. Intelligence 通过稳定查询接口读取，不扫描其他编辑器私有状态。
2. AI 只输出 Plan、Proposal、Code Patch 或受约束 Action，不直接覆盖 Workspace。
3. Embedding、vector index 和模型缓存是可重建 projection，不是作者态真相。
...
flowchart TD
    ...
    Animation --> Authoring
    NodeGraph --> Diagnostics["@prodivix/diagnostics"]
    Animation --> Diagnostics

    PIR["@prodivix/pir"] --> NodeGraph
    PIR --> Animation
    PIR --> Diagnostics
...
PIR 可以组合 NodeGraph 与 Animation 的序列化 section，但领域 contract 和 validator 归各自 package。迁移完成后，NodeGraph 与 Animation 不反向依赖完整 PIR 文档。

`apps/web/src/core` 不保留兼容 alias。现有 executor、NodeGraph bridge、空 node/worker 文件应在对应 package 和 Browser Adapter 就绪后 Hard Cut 删除。
...
### A1：Runtime Core 与 NodeGraph Hard Cut

1. 创建 `@prodivix/runtime-core`。
2. 创建 `@prodivix/nodegraph`。
3. 将 React Flow 类型从 domain model 移除。
4. 以 Executor Registry 和 deterministic trace 替换全局 singleton、Window bridge 和 `console` trace。
5. Web 创建 Browser Adapter；删除 `@/core` import 和兼容路径。

### A2：Animation Hard Cut

1. [已完成] 创建可独立 build/test 的 `@prodivix/animation`，由其拥有 contract、确定性 normalizer、作者态工厂、keyframe 与 timeline evaluator。
2. [已完成] Web 与 Compiler 共享 Animation keyframe 求值；Web 旧 model、keyframe evaluator 和 preview evaluator 已 Hard Cut。
3. [已完成] 创建 `@prodivix/runtime-browser`，承接 NodeGraph 浏览器执行适配、浏览器 ID port 与 Animation CSS/SVG preview projection。
...
4. [已完成] Resource、Route、NodeGraph、Animation 等作者态写入统一进入 WorkspaceOperation Outbox；Settings 进入独立 strong-idempotent Settings Outbox；旧 PATCH/Intent transport 已删除。
...
1. [待开始] 创建 Browser/Remote ExecutionProvider。
2. [待开始] 建立 BehaviorScenario、VerificationPlan 和 Evidence contract。
3. [进行中] Preview、Export 和 Test 接入同一 conformance suite。
4. [基线已完成] `@prodivix/golden-conformance` 已将 Golden App 建成 Living Conformance 基线，覆盖 create、edit、History、Atomic Commit save planning、Outbox/local replica recovery、显式 conflict resolution、同一 `pir-component` 的双 route 单模块复用、full-workspace React/Vite export 与进程内 Vite build。
5. [待开始] 将 Golden 扩展到独立项目 install、typecheck、test、runtime behavior、browser smoke、visual regression，并把运行结果固化为正式 `VerificationEvidence`。当前基线只逐个 syntax-transform 生成的 JS/TS 模块，并在进程内 build 时外部化 bare package imports，不应被解释为这些 Gate 已通过。

### A4：Intelligence、Agent 与 Production Feedback

1. 建立 Workspace Intelligence Index。
...
5. 不恢复旧 README 的供应商枚举和旧 Phase 1-4 完成标记。
6. 不为现有早期 NodeGraph executor 或 Animation state shape 保留兼容层。
...
2. NodeGraph 与 Animation 早期实现会被删除或重写。
3. 建立 Verification Evidence、ExecutionProvider 和 Intelligence Index 需要新的基础设施投入。
4. 在语义和 Gate 稳定前，新增表面功能的速度会主动降低。

这些成本是有意接受的。项目仍处于 alpha，当前优先级是建立长期稳定的工程内核，而不是维持早期实现的兼容性。

## 验收标准

- [x] `specs/roadmap/global-phases.md` 成为全局阶段唯一来源。
- [x] 新 RFC 与实施计划模板强制包含 Global Phase 和 Product Gate。
- [x] `apps/web/src/core` 被 Hard Cut，Web 不再拥有 transport-neutral runtime 或 NodeGraph executor。
...
- [x] Shared Router subpath 与 Web-owned PIR Renderer 已 Hard Cut，不保留兼容入口。
...
- [x] 所有生产写入完成 Operation/Settings Outbox Hard Cut，旧 PATCH/Intent 分支已删除。
...
- [ ] Golden App 在独立导出项目中完成 install、typecheck、test、browser smoke 与 visual verification。
- [ ] Preview、Test 和 Export 至少共享一组 NodeGraph/Animation conformance scenarios。
- [x] AI、插件和导入器不能绕过 Workspace Command / Operation / Validator；Hard Cut checker 持续拒绝第二写路径。
