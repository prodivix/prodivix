...
| `46.auth-and-server-runtime.md`                             | Accepted    | Remote Auth/Test + Isolated Production/Import Graph + Live Mutation + Isolated Secret A10 Verticals Implemented | current contract、canonical code profile、reference-only `/config/auth.json`、Resources/Issues/compile Gate、session-bound gateway、Remote bridge、metadata-only invocation devtools、deterministic Test/action、bounded canonical import graph/rootless Worker、安全 live mutation，以及 lease-fenced sealed isolated Secret + monotonic worker-attempt recovery first vertical 已建立。 | 第三方 provider/其他 permission、KMS/key rotation、项目源码 mutation isolated target、跨 replica artifact/quota recovery、跨 producer debugger 与完整 leak/Golden closure。 |
