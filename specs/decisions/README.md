...
| `08.route-manifest-outlet.md`                       | Accepted    | Route Core/Preview/Backend/Topology Implemented / React Layout Composition Planned     | Workspace RouteManifest、RouteRuntimeContext、Outlet preview、后端校验、Export topology 与 `routeRev` 写入链路已落地。                                                                                                           | 完成 React/Vite layout chain、route-outlet composition 与独立项目验证。                                              |
| `09.component-route-composition.md`                 | In Progress | Frontend Composition Implemented / Export Integration Planned                          | `RouteModule`、`RouteModuleMount`、合成 RouteGraph、source trace 与 `PdxRoute` route context 已落地。                                                                                                                            | 由 export planner 消费合成 route graph。                                                                             |
| `10.pir-contract-validation.md`                     | Accepted    | PIR-current Domain Active                                                              | 无版本领域模型、显式 current wire manifest、`PIRWire*` generation、codec/migration 与后端 current contract 已形成确定性边界。                                                                                                    | 以 migration conformance 持续验证数字 wire 升级不扩散到生产消费者。                                                  |
| `11.revision-partitioning.md`                       | Accepted    | Implemented                                                                            | Workspace、Route、Document revisions、canonical 409、多分区 Atomic Commit 与 Outbox 因果 head 已落地。                                                                                                                           | 让全部生产写入使用同一分区与 Outbox 契约。                                                                           |
...
| `13.route-runtime-contract.md`                      | In Progress | Frontend Foundation Implemented / Runtime Execution Planned                            | `RouteRuntimeContext`、params 注入、navigate resolver、runtime CodeReference 诊断与 loader/action/guard CodeSlotContract 已落地。                                                                                                | 建立 route runtime execution、export adapter 与后端校验链路。                                                        |
...
| `24-27 diagnostics`                                 | Draft       | G0 Issues Implemented / Provider Expansion Planned                                     | `@prodivix/diagnostics` 已承接 provider snapshot、revision、去重、presentation 与 typed Quick Fix；Web Issues 已接入主要作者态领域和 TS/JS/CSS/SCSS/GLSL/WGSL diagnostics，并支持稳定 target、Code SourceSpan 聚焦和编辑器回跳。 | 接入 Plugin、UX、Export provider 与 GPU/目标后端 shader compile validation。                                         |
| `25.authoring-symbol-environment.md`                | Accepted    | Workspace Semantic Index Kernel Implemented / Provider Integration In Progress         | Immutable snapshot、provider composition、WorkspaceSymbol/Scope/Reference、revision identity、visibility、resolution、references 与 impact query 已落地并通过属性测试。                                                          | 接入各领域 semantic contribution；Language Service 通过 contribution/capability provider 接入。                      |
| `28.code-authoring-environment.md`                  | Accepted    | Code Workspace and Language Capabilities Implemented / G1 Slot Integration In Progress | Workspace code document、CodeArtifact projection、CodeReference、CodeSlotContract/Registry、TriggerBinding 与 TS/JS/CSS/SCSS/GLSL/WGSL revision-bound session 已形成独立 package 边界。                                          | 实现 orphan 生命周期、主要 code-owned slots、跨领域 impact UI 与 controlled round-trip。                             |
...
| `31.production-export-planner.md`                   | Draft       | Planner Foundation Implemented / G1 Standalone Verification Planned                    | `ExportProgram`、`ExportProgramBuilder`、`ProductionExportPlanner`、artifact/source/origin/license/deployment metadata 与 `.prodivix/*` 审计文件已落地。                                                                         | 完成独立项目验证、多 framework target、source map、tree shaking、code splitting 和 License UI。                      |
...
| `34.core-package-boundaries.md`                     | Accepted    | G0 Core Extracted / G1 Semantic Composition Active                                     | Runtime、NodeGraph、Animation、Router、Browser Runtime、PIR React Renderer、Workspace Outbox、local replica 与 Workspace Semantic Index 已有稳定 owner 与 revision-bound composition。                                           | 接入 Code Language、Token、typed port/executor 贡献，并继续 Shared 收缩。                                            |
| `35.canonical-workspace-hard-cut.md`                | Accepted    | Implemented                                                                            | Canonical WorkspaceSnapshot、wire codec/settings envelope、Store 单快照、confirmed revision/edit sequence、Command/Transaction、route-aware active document 与严格加载门禁已落地。                                               | 让 G1 Component、Collection、Code 与 Semantic Index 继续复用同一 canonical boundary。                                |
...
| `37.verified-semantic-authoring-architecture.md`    | Accepted    | G0 Kernel Implemented / G1-G6 Expansion Planned                                        | 七个能力平面与 ChangeSet 定位已冻结；Workspace Semantic Index 属于 Plane 2/G1，Intelligence 消费其稳定查询；G0 验证入口完整通过。                                                                                                | 建设 Semantic Index、Component/Collection、Language Service、visual/code round-trip、ExecutionProvider 与 Evidence。 |
| `38.blueprint-component-instance-and-collection.md` | Accepted    | S4 Implemented / Product Surface In Progress                                           | Component/Collection contract、semantic identities、DAG、原子 extraction、History、Preview/Compiler parity 与 SourceTrace 已落地。                                                                                               | 完成 Blueprint 产品表面与 G1 Golden Gate。                                                                           |
| `39.pir-current-evolution.md`                       | Accepted    | PIR-current Architecture Active                                                        | G1 生产消费者统一使用无版本 current model；数字版本集中在 immutable wire snapshot、activation manifest、codec 与 migration 边界。                                                                                                | 以每次 wire 升级的空生产消费者 diff 作为低成本演进门禁。                                                             |
