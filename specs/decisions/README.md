...
| `24-27 diagnostics`                                 | Draft       | G0 Issues + G1 Language/Shader Diagnostics Implemented / Provider Expansion Planned   | `@prodivix/diagnostics` 已承接 provider snapshot、revision、去重、presentation 与 typed Quick Fix；Web Issues 已接入主要作者态领域和 TS/JS/CSS/SCSS/GLSL/WGSL diagnostics、独立 shader compile，并支持稳定 target、Code SourceSpan 聚焦和编辑器回跳。                                     | 接入 Plugin、UX、Export 与 G2 Execution provider。                                                  |
| `25.authoring-symbol-environment.md`                | Accepted    | Workspace Semantic Index + Core Domain Provider Composition Implemented               | Immutable snapshot、revision identity、visibility、resolution、references、impact query，以及 Workspace/Route/PIR/NodeGraph/Animation/Code/Token/Resolver/Asset provider composition 已落地并通过属性测试。                                                                               | 按新领域扩展 typed contribution，并推进 Language Service worker lifecycle。                         |
...
| `34.core-package-boundaries.md`                     | Accepted    | G0 Core Extracted / G1 Semantic Composition Implemented                               | Runtime、NodeGraph、Animation、Router、Browser Runtime、PIR React Renderer、Workspace Outbox、local replica 与 Workspace Semantic Index 已有稳定 owner 与 revision-bound composition。                                                                                                    | 让 G2 Execution、Data 与 runner 能力继续遵守 package owner 边界。                                   |
...
| `37.verified-semantic-authoring-architecture.md`    | Accepted    | G0 Kernel + G1 Semantic Hybrid Authoring Implemented                                  | 七个能力平面已冻结；Semantic Index、Language、Component/Collection、controlled round-trip 与 standalone export/browser Gate 已落地。                                                                                                                                                      | 建设 G2 ExecutionProvider 与 G3 正式 VerificationEvidence。                                         |
