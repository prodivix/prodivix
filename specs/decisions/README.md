...
| `06.command-history.md`                             | Accepted             | Local History Implemented                            | Operation/Transaction History、scope/barrier、merge window、因果 undo/redo、Store 原子接线和四类编辑器快捷键已实现；CodeMirror 保留原生文本 History；Resource/Route/NodeGraph/Animation 已接入 Operation planner。                                                                                                                     | 继续建设跨领域 ChangeSet/Verification 与 local replica 恢复后的 History 协同。                                                |
| `07.workspace-sync.md`                              | Accepted             | Write Hard Cut + Local Replica Implemented           | semantic diff3/conflict、Atomic Commit、Operation/Settings 双 Outbox、ACK causality、正式 local replica、离线 materialization 与所有生产写入接线已落地；旧 PATCH/Intent 后端分支已删除。                                                                                                                                               | 推进多端实时订阅、presence、review 与按领域选择的 CRDT/typed transaction 协作。                                               |
...
| `11.revision-partitioning.md`                       | Accepted             | Implemented                                          | Workspace、Route、Document revisions、WKS-4001/4002/4003 canonical 409、多分区 Atomic Commit 与 Outbox 因果 head 已落地；`HYBRID_CONFLICT` 已 Hard Cut。                                                                                                                                                                               | 剩余生产写入统一进入相同分区与 Outbox 契约。                                                                                  |
| `12.intent-command-extension.md`                    | Partially Superseded | Command Contract Retained / Intent Transport Removed | `WorkspaceCommandEnvelope`、patch policy 与回放校验继续生效；Intent 只作为本地/AI planner 输入，不进入 wire。旧 `POST /intents` 与服务端 handler registry 已 Hard Cut。                                                                                                                                                                | 新领域先规划为可逆 Command/Transaction，再进入 Atomic Commit。                                                                |
...
| `24-27 diagnostics`                                 | Draft                | G0 Issues Implemented / Broader Providers Partial    | `@prodivix/diagnostics` 已承接 presentation engine、provider snapshot、单调 revision、active/stale/resolved、跨 provider 去重与 typed Quick Fix reference；Web Issues 已接入 Workspace/Route/PIR/NodeGraph/Animation/Code/Outbox/Conflict，支持稳定跨文档 target、真实 Code parser diagnostics、Code SourceSpan 精确聚焦和编辑器回跳。 | 继续接入 Plugin/UX/Export provider 与真实 SCSS/GLSL/WGSL Language Service。                                                   |
...
| `30.react-flow-nodegraph-editor.md`                 | Accepted             | Editor Surface + Runtime Kernel Extracted            | React Flow 编辑器与 PIR 序列化继续作为 Web 作者表面；`@prodivix/nodegraph` 已承接无 DOM contract、strict decoder、Executor Registry、step budget 与 deterministic trace。                                                                                                                                                              | 扩展 typed control/data flow、async/error/cancel、CodeSlot executor 与 Preview/Export conformance。                           |
...
| `34.core-package-boundaries.md`                     | Accepted             | Core/Router/Renderer/Replica Hard Cuts Implemented   | Runtime、NodeGraph、Animation、Router、Browser Runtime、PIR React Renderer、Workspace 双 Outbox 与 local replica Core 已建立稳定 owner；旧写 transport 已删除。                                                                                                                                                                        | 聚焦 ExecutionProvider、Animation lifecycle/composition 与 Shared 收缩。                                                      |
...
| `36.atomic-workspace-operation-commit.md`           | Accepted             | Implemented                                          | Exact expected-vector planner、统一 Operation transport、后端单数据库事务、强幂等 replay、Resource domain、聚合 delta、Durable Outbox 与旧 `/batch`/PATCH/Intent Hard Cut 已落地。                                                                                                                                                     | 保持唯一作者态远端写边界，并扩展已注册领域的验证语义。                                                                        |
| `37.verified-semantic-authoring-architecture.md`    | Accepted             | G0 Kernel Implemented / Broader Architecture Partial | 七个能力平面与 ChangeSet 定位已冻结；核心领域 package、生产写入 Hard Cut、双 Durable Outbox/ACK causality、正式 local replica、Issues 与 Golden 已落地，G0 验证入口完整通过。                                                                                                                                                          | 进入 G1，建设真实 Language Service、visual/code round-trip、Animation lifecycle、ExecutionProvider 与 Verification Evidence。 |
