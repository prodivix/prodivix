...
- Runtime filesystem observation 使用独立 `ExecutionFilesystemDiff` artifact：Worker 在 main process 结束后先
  ...
  Web 只按授权 resolver 下载，Files 表面默认零选择。modified/deleted 要求单一 whole-file CodeArtifact trace、
  exact partition revision、baseline byte 与 UTF-8 全通过；deleted 还要求 exact Workspace/Route revision、非 active
  lifecycle 和 canonical delete Intent preflight。added 仅允许受支持 CodeArtifact 扩展名、完整 UTF-8、无 SourceTrace、
  exact Workspace revision 与无冲突 path/id，并通过 canonical create Intent preflight。显式选择按 add/modify/delete
  确定性规划成一个可逆 Workspace Transaction，经 Durable Outbox/Atomic Commit 写入；partial/aggregated、
  unknown/binary/stale/conflict、active lifecycle、controlled PIR projection 与 incomplete capture 均 fail closed。
