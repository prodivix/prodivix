...
- `digest / size / sourceTrace`：有独立 artifact bytes 时由 adapter 发布并由 resolver 校验；Browser report
  的 canonical content 已在同一 `test.report` trace 中，因此 artifact 只承担 correlation/source metadata，
  不伪造第二份可下载 bytes；
- locator：需要独立 bytes 时，调用方以 `artifactId + provider/session identity` 通过授权 artifact resolver 获取。
  `uri` 只能是可选、短生命周期、opaque adapter locator，不得把 Browser scheme 或 Remote 供应商 URL
  固化为 shared contract。

Browser Test report artifact 不再发布 `browser-project://` URI，Web 也不认知 adapter scheme；Remote 与未来
独立 attachment bytes 继续走授权 resolver。
...
Adapter 对原始报告字符数、file/case 数、failure message 与 SourceTrace 数量设置确定性上限；Core 在
transport decode 时再次执行 exact-key、UTF-8 byte、file/case/message/SourceTrace budget。超过可安全保留的报告
直接作为 `TST-5002` 基础设施失败，不把截断提示伪装成断言失败，也不向有界 Session history 写入无限
report detail。cancel/timeout 只保留 Job terminal outcome；未形成完整 canonical report 时不发布 artifact/trace，
partial report 不可表达。
...
- [x] Browser report 通过 canonical trace消费，correlation artifact不再携带 adapter URI；独立 bytes只通过 resolver获取。
