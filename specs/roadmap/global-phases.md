...
- transport-neutral Data trigger origin、typed input mapping、query activation equality、mutation dispatch identity replay fencing、Browser composition、PIR/Inspector v1.6 durable trigger authoring，以及 Preview/Export shared React/Vite mock trigger/CRUD runtime、standalone public client live/policy runtime、Backend production Environment/Secret store first vertical、Remote create authority binding、execution-bound HTTP/material query/mutation gateway、durable replay fence、显式 upstream idempotency/retry、generated Remote Preview query/mutation bridge/CSP、Remote Network/Console Session correlation、manual new-request recovery、Structured Console copy Gate、Remote Terminal transport-wide/cross-chunk Gate、runtime FS diff/whole-file CodeArtifact add/modify/delete VFS proposal 与当前 Remote durable 输出 canary Gate 已完成；完整 Remote failure recovery、KMS/key rotation、OpenAPI/GraphQL/AsyncAPI adapter，以及第二 target CRUD parity 尚未完成。
...
- 真实 rootless Remote Golden CRUD parity、Structured Console/Files 完整 SourceTrace 调试旅程与单一第二 target 可移植性证明继续作为 G2 后续交付；广泛 Multi-target/Target SDK 生态属于 G6。当前 rootless runtime FS 已通过 capture-ready/PTTY-close handshake 形成 strict bounded diff artifact；Worker 对照 exact snapshot 重建 baseline/SourceTrace，Execution Center Files 仅允许用户显式采纳 revision/lifecycle-fenced whole-file CodeArtifact add/modify/delete，并按确定性顺序形成单个可逆 Workspace Transaction 进入 Outbox/Atomic Commit。
