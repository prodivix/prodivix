...
conmon；Linux controlled static install 已将 60 秒 frozen lockfile-only 元数据校验与 image-owned immutable
package seed 物化分离，同时配置 Chromium/Firefox/WebKit 与真实 rootless Podman 执行；远端 CI evidence 尚待固定
commit/run identity。
