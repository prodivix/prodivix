...
| 2    | `pnpm run check:editor-hard-cut`               | 扫描生产源码，拒绝退休的 Web-owned Core/PIR renderer、Workspace/PIR 镜像、旧 Intent/PATCH transport、绕过 Outbox 的 Atomic Commit、未审计浏览器持久化与后端旧写路径。 |
...
| 2   | Blueprint、NodeGraph、Animation、Route、Code、Resources 写入全部进入 Command / Transaction | 阶段 2、5、6、7；Hard Cut 拒绝直接 transport、旧 Intent/PATCH、私有镜像，Workspace tests 验证各领域 planner/command。                                                 | Passed；Hard Cut 与领域测试同时通过。                                                     |
...
| 8   | Golden 覆盖多路由、复用组件、表单、代码、资源、插件、导出和冲突恢复                        | 阶段 5；`goldenApp.conformance.test.ts`、`goldenApp.property.test.ts`、`workspaceExportContracts.conformance.test.ts`。                                               | Passed；全部 Golden 正向覆盖、确定性与 fail-closed contract 通过。                        |
...
| 复用组件   | 同一 `pir-component` artifact 被至少两个作者态消费点引用，导出只生成一个共享组件模块，消费点复用该模块。                 | 只创建未被消费的 component；或仅由 fail-closed 测试拒绝尚未支持的 route outlet/layout composition。 |
...
复用组件是 G0 Golden 的独立正向要求。`goldenApp.conformance.test.ts` 已证明同一 `pir-component` 被 `/order-summary` 与 `/order-summary-preview` 两个 route 消费，导出只产生一个共享组件模块和一个对应 import；`workspaceExportContracts.conformance.test.ts` 则继续独立证明尚未支持的组合不会静默丢失。
...
| 3   | 不存在绕过 Command / Transaction 的第二套生产写入协议                      | Editor Hard Cut 扫描 raw commit transport、旧 Intent/PATCH、Project PIR mirror、后端旧 endpoint；Core boundary 阻断 Web-owned domain Core。 | Passed；阶段 1、2 成功且无语义写入 allowlist。       |
...
2. Golden 具有复用组件的正向双消费、单模块证据，而不仅是 fail-closed 负向证据。
