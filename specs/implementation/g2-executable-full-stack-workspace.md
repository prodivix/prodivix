...
| Binary Asset                  | ADR 47、`@prodivix/assets` strict reference/materialization/transform pipeline、Workspace reference-only hard cut、PostgreSQL/IndexedDB blob、local/cloud lifecycle、Run/Test/Export/Remote exact bytes、PNG/JPEG structural sanitizer + bounded Sharp full-raster、required ClamAV/YARA-X chain、replica/freshness/policy generation、bounded derived cache、Backend owner gateway、capability Asset Delivery Host、Browser JPEG product journey、retention、Git/LFS、runtime import/replace，以及 React/Vue Browser/Test/Remote Preview/Test/Build matrix与 protected static fail-close已本地实现；旧 ClamAV real-daemon evidence已通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 新 ClamAV + YARA-X Actions evidence待推送；更多格式/额外 vendor/durable CDN是 post-G2 adapter范围                                                            |
| Auth / Route / Server Runtime | ADR 46、A0-A13/A15/A16 已实现并取得 GitHub rootless/PostgreSQL evidence；A17 editor/sharing 产品纵切与本机 PostgreSQL Gate 已完成；A14 AWS managed KMS official adapter、本地/PostgreSQL Gate 与 OIDC workflow 已配置；当前 G2 Auth/Secret surface matrix 与 Golden closure 已通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | A14 live AWS与A17本轮 CI evidence待取得；其他 permission/role/provider属于 post-G2                                                                           |
...
| Binary Asset pipeline                             | ADR 47 + `g2-binary-asset-pipeline.md`            | B0-B7 local/cloud/full-raster/required engines/retention/Git/runtime import/Browser journey/React+Vue target matrix locally implemented                                 | new ClamAV+YARA-X CI evidence；extra formats/vendor/CDN promotion are post-G2 |
| Auth/session/permission/server function           | ADR 46 + `g2-auth-server-runtime.md`              | A0-A13/A15-A17 current-scope local closure；A14 configured / live evidence pending                                                                                      | A14 live AWS、A17 current-worktree CI；future roles/providers are post-G2     |
...
import/replace，以及 B7 Browser JPEG与 React/Vue exact-byte target matrix。旧 ClamAV real-daemon证据已通过；新增
ClamAV + YARA-X workflow证据待本轮修改后续推送取得。更多格式、额外 vendor与 durable public-CDN promotion是
...
状态：Local closure / external evidence pending；Auth/Server A0-A13/A15/A16已实现并取得既有远端证据，A17 editor
sharing vertical本地完成，A14 AWS live evidence待取得；Binary Asset B0-B7 local/cloud blob、full-raster、required
ClamAV/YARA-X、retention/Git/LFS/runtime import、Browser JPEG与 React/Vue target matrix本地完成，新增 real-engine CI证据待取得。
...
Binary Asset 只待新增 ClamAV + YARA-X rootless workflow远端证据；旧 ClamAV real-daemon evidence已存在。更多格式、
额外 vendor与 durable public-CDN publish/purge是 post-G2 adapter/deployment范围，不能通过恢复 inline data URL兼容层实现。
...
A14 live AWS与本轮 A17/双 malware engine CI evidence 仍待外部运行；其他 permission/role、其他 managed-cloud KMS adapter、
...
`specs/roadmap/g2-closure-evidence.md`；真实云、当前 worktree rootless/双引擎/A17 CI 证据落地前不得标记 G2 Passed。
...
- [x] Binary Asset full-raster/required-engine delivery、React/Vue cross-target product journey与 Auth/Server Runtime
      local安全 Gate完成；新增 real-engine CI与 AWS live evidence单独保持 pending。
...
- [x] Golden G2 在 Browser、Remote、Test 与 standalone React/Vue Export 的本地 contract/product Gate 中通过；当前 worktree rootless CI evidence独立 pending。
