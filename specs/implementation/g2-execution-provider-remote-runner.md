...
- [x] controlled static rootless install 将 frozen lockfile 的纯元数据校验与 immutable package seed
      物化硬分离：image build 先以真实 frozen/offline `pnpm install` 验证 exact fixture manifest/lock 并
      生成 seed；runtime install 不再调用 package manager，而由 30 秒上限的 Node authority command 在
      `network=none`、最小环境中重算 control、manifest、lock、workspace、Vite config 与 isolation probe
      的 exact digest/file-set identity。校验前后均不得出现 `node_modules` 或 toolchain bytes 漂移，
      通过后才按 image-owned seed authority 解包、逐项校验与最终 rehash。
