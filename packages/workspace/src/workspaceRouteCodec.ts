...
import type { WorkspaceDocumentType } from './types';
...
export type WorkspaceRouteDocumentTypeResolver = (
  documentId: string
) => WorkspaceDocumentType | undefined;

export type WorkspaceRouteManifestDecodeOptions = Readonly<{
  resolveDocumentType?: WorkspaceRouteDocumentTypeResolver;
  /** @deprecated Migrate to resolveDocumentType so route roles can be checked. */
  documentExists?: (documentId: string) => boolean;
}>;

export type WorkspaceRouteManifestDecodeInput =
  WorkspaceRouteManifestDecodeOptions | ((documentId: string) => boolean);
...
const requireRouteSegmentString = (value: unknown, path: string): string => {
  if (typeof value !== 'string') {
    throw new WorkspaceCodecError(path, 'Expected a string.');
  }
  return value;
};
...
const validateCanonicalRouteManifestStructure = (
  ...
  options: WorkspaceRouteManifestDecodeOptions
): void => {
  ...
  const validateDocumentReference = (
    documentId: string,
    path: string,
    allowedTypes: ReadonlySet<WorkspaceDocumentType>,
    role: string
  ): void => {
  ...
  const pageDocumentTypes: ReadonlySet<WorkspaceDocumentType> = new Set([
    'pir-page',
    'pir-component',
  ]);
  const layoutDocumentTypes: ReadonlySet<WorkspaceDocumentType> = new Set([
    'pir-layout',
  ]);
...
export const decodeWorkspaceRouteManifest = (
  value: unknown,
  input: WorkspaceRouteManifestDecodeInput = {}
): WorkspaceRouteManifest => {
  const options: WorkspaceRouteManifestDecodeOptions =
