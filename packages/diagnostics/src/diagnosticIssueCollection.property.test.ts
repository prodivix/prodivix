...
const propertyTestTimeoutMs = 20_000;
...
describe('diagnostic issue collection properties', () => {
  it(
    'is independent of provider insertion order at one revision',
    () => {
      fc.assert(
        fc.property(
          fc.uniqueArray(
            ...
            }),
            {
              minLength: 1,
              maxLength: 8,
              selector: (value) => value.providerId,
            }
          ),
          (providers) => {
  ...
  it(
    'deduplicates equal locations while preserving provider occurrences',
    () => {
      fc.assert(
        fc.property(
          diagnosticArbitrary,
          ...
          (value, firstCount, secondCount) => {
            ...
            const active = queryDiagnosticIssues(state, {
              statuses: ['active'],
            });
