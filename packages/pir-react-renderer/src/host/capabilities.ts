...
export type NodeCapability = {
  key: string;
  match: (node: PIRElementNode) => boolean;
  link?: LinkCapability;
};
 ...
 */
export const resolveNodeCapabilities = (node: PIRElementNode | null) => {
  if (!node) return [];
  return capabilityRegistry.filter((capability) => capability.match(node));
};

export const resolveLinkCapability = (node: PIRElementNode | null) =>
