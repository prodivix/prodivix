...
func rewindAgentEvaluationFinalizationAuthorityMigrations(
	...
	for _, statement := range []string{
		...
		`ALTER TABLE agent_evaluation_authority_attestations
			DROP COLUMN IF EXISTS v46_eligible,
			DROP COLUMN IF EXISTS v45_eligible`,
		`ALTER TABLE agent_evaluation_evidence_roots
			DROP CONSTRAINT IF EXISTS agent_evaluation_evidence_roots_v45_exact_identity_key CASCADE,
			DROP COLUMN IF EXISTS v46_eligible,
			DROP COLUMN IF EXISTS v45_eligible`,
		`ALTER TABLE agent_evaluation_controlled_authority_requests
			DROP CONSTRAINT IF EXISTS agent_evaluation_controlled_authority_v45_stage_check,
			DROP CONSTRAINT IF EXISTS agent_evaluation_controlled_authority_v45_response_check,
			DROP CONSTRAINT IF EXISTS agent_eval_controlled_authority_v45_g3_cell_admission_check,
			DROP COLUMN IF EXISTS v46_eligible,
			DROP COLUMN IF EXISTS v45_eligible,
