...
import { createPirCurrentProjectionPlan } from '@/plugins/platform/__tests__/pirCurrentRenderer.fixture';
...
const createPlatform = () => {
  ...
  if (result.ok === false) {
    throw new Error(
      'Official plugin contract fixture platform must initialize.'
    );
  }
    ...
    graph: {
      ...
      nodesById: {
        'neutral-button': {
          ...
          kind: 'element',
          type: 'NeutralButton',
          text: { kind: 'literal', value: 'Launch' },
        },
      },
...
describe('official plugin contracts', () => {
  ...
  it('completes Palette -> Canvas without a library-specific Host branch', async () => {
    const platform = createPlatform();
    ...
    render(
      ...
      >
        <div className="relative">
          <OfficialReactSurfaceBoundary>
            <PIRRenderer
              plan={createPirCurrentProjectionPlan(createButtonDocument())}
              host={host}
