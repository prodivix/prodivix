...
Prodivix 使用稳定错误码帮助定位问题。每个错误码都有独立的说明页，可用于快速理解其含义、确认触发条件并找到建议的处理方式。

## 如何使用

1. 在界面或响应中找到稳定错误码，例如 `WKS-4003`。
2. 打开对应的错误码页面，先看严重程度、阶段和触发条件。
3. 按建议操作进行修复。若需上报，请使用下方模板。
...
请勿上报 API key、Token、完整 Prompt 或其他敏感内容。
...
| `AI-xxxx`  | AI 助手            | Provider/Capability、Context/Media、Task/Run、Tool/Retrieval、Approval 和 Evaluation/Verification |
...
后端 API 的错误响应会将稳定错误码放在 `error.code` 字段中，并可能同时返回 `requestId`。
