...
Prodivix 使用 pnpm workspace 与 Turborepo。应用层负责组合与交互界面，稳定的领域能力归属于独立 package。
...
- `specs/roadmap/global-phases.md` 是全局阶段定义的唯一来源。
- `specs/decisions/` 记录稳定架构契约，产品文档不复制完整协议正文。

## Package 依赖方向

领域 package 应保持 transport-neutral 和 DOM-free；浏览器、React 或后端适配位于边界层。Web 可以组合 package，但 package 不应反向导入 Web 内部实现。
...
完整的 owner 表、修改落点和依赖规则见[架构与 Package Owner](/developer/architecture)。
