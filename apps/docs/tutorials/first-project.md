...
本教程带你完成一次最小但完整的作者闭环：创建项目、编辑 Blueprint、检查诊断、使用 History，并查看生产导出。

## 1. 启动并创建项目

先按[本地启动](/guide/getting-started)运行 Web 编辑器。在首页创建一个本地项目，并进入项目工作区。

本地项目使用正式 Workspace replica 与 outbox adapter。它并非仅存在于组件 state 或 `localStorage` 中的临时草稿。
...
如果节点被其他内容遮挡，可在组件树中临时隐藏它。该按钮只控制作者画布，不影响预览和导出结果。
...
History 回放的是领域 Command/Transaction，而非 DOM 快照还原。

## 4. 查看 Resources

打开 Code Workspace，编辑项目级 Workspace 代码文档。Resources 汇总了项目文件、外部依赖、公共资源、国际化资源、Design Token 和可复用组件，并为导入文件、外部文件及资源归属的代码文件提供完整的编辑入口。

选中某个代码文档后，代码编辑器和语言能力会绑定到当前 Workspace revision。诊断、跳转和重命名建议不会直接覆盖其他领域文档。
...
1. 选择一条诊断，查看其目标、来源和严重程度。
2. 使用“打开目标”或“打开文档”回到对应作者表面。
3. 只有提供稳定修复契约的诊断才会显示 Quick Fix。

点击错误图标可复制错误详情；复制反馈保持中性样式，不代表问题已经修复。

## 6. 查看导出计划

打开 Export，选择当前已验证的 React/Vite target。导出前应先处理会阻止生成的错误，然后检查源码、样式、运行时、资源、配置和依赖计划。
