...
hero:
  ...
  tagline: 在浏览器中组合 Blueprint、NodeGraph、Animation、Code、Diagnostics 与生产导出，而非将项目压成一个巨型 JSON。
  image:
    src: /logo.svg
    alt: Prodivix
  actions:
...
features:
  ...
  - title: Durable Change
    details: 编辑操作经由可逆 Command 或 Transaction，依次进入 History、Durable Outbox 与 Atomic Commit。
  - title: 可验证导出
    details: 当前 Golden target 是 React/Vite，已具备独立 install、typecheck、test、build 与浏览器 Gate。
---
...
::: warning Alpha 边界
Prodivix 尚未承诺生产稳定性。Workspace Test 已可运行 exact export snapshot，但尚不等同于 G3 VerificationEvidence；Deployment、完整 Data/API lifecycle、多框架 target、远程执行与团队协作仍在按各自 Gate 推进。
:::
...
| 理解为何不会出现第二个真相来源 | [Canonical Workspace VFS](/concepts/workspace-vfs)        |
...
视觉表面、代码编辑器、Issues 和 AI proposal 都复用同一条链路；没有任何入口可以直接覆盖另一个编辑器的私有状态。
